...
class AirSummary {
    ...
    getAlerts(aq, user){
        ...
        return aq.aqius <= 50 ? []: [
            "🏭"+getAlertLevel(aqius, user.thresholds.aqi)+" AQI "+aq.aqius+", PM2.5 "+aq.pm25+"µm/m3",
...
class TempSummary {
    ...
    getColdAlert(avg, min, thresholds){
        ...
        if(alertLvl.includes("🔴"))
            return "🌡️🥶🔴 Put winter gear";
        if(alertLvl.includes("🟠"))
            return "🌡️🥶🟠 You may need winter gear, watch out for ice🧊";
        if(alertLvl.includes("🟡"))
            return "🌡️🥶🟡 You may need light jacket/sleeves";
    }
}
